...
output$sc1c2oup.png <- downloadHandler( 
  ...
  output$cytoplot <- renderImage({
    gcyto(input$cytoinp)
    list(src = 'cytoplot.png')
  })
  output$cytoplotu <- renderUI({ 
    imageOutput("cytoplot") 
  }) 
  output$msplot <- renderImage({
    gms(input$msinp)
    list(src = 'msplot.png')
  })
  output$msplotu <- renderUI({ 
    imageOutput("msplot") 
  }) 
  ### Plots for tab d1 
...
reticulate::virtualenv_install("python35_env", packages = c("matplotlib"))
reticulate::use_virtualenv("python35_env", required = TRUE)
source_python('cytopy.py')
