...
### Page title 
titlePanel("Human iPSC-derived microglia (iMG)"),  
navbarPage( 
  NULL,  
 ### Tab1.a1: cellInfo vs geneExpr on dimRed 
  tabPanel( 
    ...
    "expression side-by-side on low-dimensional representions. Page may take some time to load.", 
    br(),br(), 
    fluidRow( 
      column( 
 ...
 tabPanel( 
   ...
   "(e.g. Number of UMIs / module score) across groups of cells (e.g. library / clusters).", 
   br(),br(), 
   fluidRow( 
     column( 
  ...
  fluidRow( 
    column( 
      ...
      radioButtons("sc1c2typ", "Plot value:", 
                   ...
                   selected = "CellNumbers", inline = TRUE), 
...
tabPanel( 
  HTML("Cytokine plot"), 
  h4("Fold change of cytokines when cells are exposed to LPS"), 
  "For the analysis of cytokine/chemokine changes, the fold change of the cytokines/chemokines was calculated by comparing the fluorescent intensities of the cytokines before and after the addition of LPS.",
  br(),br(), 
  fluidRow(
    
    column( 
      3, 
      textAreaInput("cytoinp", HTML("List of Cytokine names <br /> 
                                          "), 
                    height = "200px", 
                    value = paste0('EGF, Eotaxin, FGF-2, FLT-3L, Fractalkine, G-CSF, GM-CSF, GROα, IFN-α2, IFNγ, IL-1α, IL-1β, IL-1RA, IL-2, IL-3, IL-4, IL-5, IL-6, IL-7, IL-8, IL-9, IL-10, IL-12p40, IL-12p70, IL-13, IL-15, IL-17A, IL-17E/IL-25, IL-17F, IL-18, IL-22, IL-27, IP-10, MCP-1, MCP-3, M-CSF, MDC, MIG/CXCL9, MIP-1α, MIP-1β, PDGF-AA, PDGF-AB/BB, RANTES, TGFα, TNFα, TNFβ, VEGF-A, 6CKine, BCA-1, CTACK, ENA-78, Eotaxin-2, Eotaxin-3, I-309, IL-16, IL-20, IL-21, IL-23, IL-28A, IL-33, LIF, MCP-2, MCP-4, MIP-1δ, SCF, SDF-1α+β, TARC, TPO, TRAIL, TSLP', collapse = ", ")) %>% 
      
      fluidPage(
        uiOutput(outputId = "cytoplotu"))
...
tabPanel( 
  HTML("Morphology"), 
  h4("Morphology"), 
  "A: Immunocytochemistry (ICC) of IBA1 and TEMEM119 expression under near homeostatic conditions in iMG derived from indicated iPSC lines. Scales bars represent 20 µm.
B. qPCR data shows the expression of microglial signature genes in iMG derived from three different iPSCs lines. Color and style denote p-values and iPSC line, respectively, with the X-axis displaying the log2 fold change.
CU-iPSC: A previously published male iPSC line from The Stem Cell Core Facility at Columbia University
C1-iPSC: a commercially procured line from male bone marrow CD34+ cells using Sendai virus
C2-iPSC: a commercially available human episomal iPSC line, derived with zero footprint from the CD34+ cord blood cells of a female individual using an EBNA-based episomal system.",
  br(),br(), 
  fluidRow(

        fluidPage(
          img(src = 'Morphology for WB-1.png', height = '1420px', width = '1000px')
...
tabPanel( 
  HTML("Ataq-Seq UCSC Genome Browser"), 
  tags$a(href="http://genome-euro.ucsc.edu/s/jasonchungngo/IPSCderivedMicroglia", "Ataq-Seq UCSC Genome Browser link"),
  br(),br(), 
  fluidRow(
...
tabPanel( 
  HTML("Proteomics plot"), 
  h4("Mass spectrometry abundance"), 
  "The intensity of individual abundances was divided by the sum of all abundances to normalize the data.",
  br(),br(), 
  fluidRow(
    
    column( 
      3, 
      textAreaInput("msinp", HTML("List of Protein names <br /> 
                                          "), 
                    height = "200px", 
                    value = paste0('ABI3, ADAM10, APOC1, APOE, BCAM, BCKDK, BIN1, BLOC1S3, CD2AP, CD33, CELF1, CLPTM1, CLU, CR1, DSG2, FERMT2, INPP5D, KAT8, MEF2C, PICALM, PLCG2, PTK2B, PVR, PVRL2, RIN1, SORL1, TOMM40, TREM2, TYROBP', collapse = ", ")) %>% 
      
      fluidPage(
        uiOutput(outputId = "msplotu"))
    )
  )
),
...
p(HTML('&nbsp;'),HTML('&nbsp;'),em("This webpage was made using "), a("ShinyCell", 
